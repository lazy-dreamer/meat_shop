        ...
        <div className="cart_sides">
          <div className="cart_side content_side">
            ...
            <div className="cart_blocks bottom_offset">
              {
                cartItems.length>0 ? (cartItems.map((item, index) => <CartItem key={index} deleteItem={deleteCartItem} item={item} />)) : <p className="empty_cart_message">Корзина пуста!</p>
              }
            </div>
            ...
            <div className="controll_buttons">
              ...
              {
                cartItems.length>0 && (<Link className="main_btn" to="/checkout"><span className="main_btn_inner">Перейти к оформлению</span></Link>)
              }
            </div>
          ...
          <div className="cart_side aside_side">
            <div className="aside_side_frame">
              <div className="aside_side_frame_title">{cartItems.length>0? (`В вашей корзине ${cartItems.length} товара`) : "Корзина пуста!"}</div>
              <ul className="price_list bottom_bordered top_bordered">
                ...
                <li className="price_li_sides">
                  <p>Скидка:</p>
                  <p>
                    {promoCodeActivate && <button type="button" className="remove_discount" onClick={()=>removeDiscount()}>(удалить код  {cartDiscountData[0]})</button>}
            ...
            {!promoCodeActivate && <div className="aside_side_frame">
              ...
              <form className="promo_frame" onSubmit={(e)=>promoSubmit(e)} action="">
                <input className={promoCodeError?"promo_field error":"promo_field"} type="text" placeholder="Введите промокод " name="" value={promoCodeField} onChange={(e) => setPromoCodeField(e.target.value.toLowerCase())}/>
